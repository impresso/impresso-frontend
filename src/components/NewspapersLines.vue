  ...
  <div v-on:mousemove="onMousemove" v-on:mouseout="onMouseout" ref="lines" class="lines small">
    <tooltip v-bind:tooltip="tooltip">
      <div v-if="tooltip.item">
        ...
        <p>
          <span
            v-if="tooltip.item.startYear != tooltip.item.endYear"
            v-html="
              $t('dates.publicationLifespan', {
    ...
    >
      <label
        :class="{ 'font-weight-bold': newspaper.included }"
        :style="{ maxWidth: margin.left + 'px' }"
      >
        {{ newspaper.name }}
      </label>
      <div
        v-if="!isNaN(newspaper.startYear) && !isNaN(newspaper.endYear)"
        class="line"
        :style="newspaperStyle(newspaper)"
      >
      ...
      <div
        v-if="newspaper.firstIssue"
        class="line included"
        :style="newspaperIncludedStyle(newspaper)"
      />
...
export default {
  ...
  computed: {
    ...
    domain() {
      return [d3.min(this.newspapers, d => d.startYear), d3.max(this.newspapers, d => d.endYear)]
    }
  },
  watch: {
    highlight: {
