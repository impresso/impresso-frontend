  ...
  <div id="thumbnail-slider" ref="thumbnailSlider">
    <div class="tiles" v-if="page && issue">
      <div
        ...
        v-bind:key="index"
        class="tile"
        v-bind:ref="`page-${item.id}`"
        v-bind:class="{ active: page.id === item.id }"
        v-on:click="onClickPage(item)"
      >
        <div class="mini_viewer" v-bind:class="{ selected: page.id === item.id }">
          <thumbnail-slider-item
            v-bind:tileSources="item.iiif"
            v-bind:bounds="bounds"
            v-bind:active="page.id === item.id && displayMode === 'image'"
          ></thumbnail-slider-item>
        </div>
...
export default {
  ...
  methods: {
    ...
    scrollToActivePage() {
      if (this.page) {
        const elm = this.$refs[`page-${this.page.id}`][0]
