...
export default {
  ...
  methods: {
    translatePeriod(duration = 0) {
      ...
      const posx = Math.min(
        Math.max(this.halfWidthInterval, this.svg.period.x),
        this.width - this.halfWidthInterval
      )
      ...
      this.svg.period.transition().duration(duration).attr('transform', `translate(${posx}, 0)`)
    ...
    updateTimeBoundaries() {
      ...
      // update day in Px
      this.app.oneDayInPx = Math.abs(
        this.app.scaleTime(this.app.date) - this.app.scaleTime(d3.utcDay.offset(this.app.date, 1))
      )
    ...
    updateTimeSelection() {
      ...
      this.app.selectionStartDateFieldValue.html(this.timeFormat(this.app.selectionStartDate))
      this.app.selectionStartDateField.attr('style', `transform: translate(${startposx}px,0)`)

      this.app.selectionEndDateFieldValue.html(this.timeFormat(this.app.selectionEndDate))
      this.app.selectionEndDateField.attr('style', `transform: translate(${endposx}px,0)`)
    },
    afterResize() {
  ...
  mounted() {
    ...
    this.app.selectionStartDateField = this.app.select('div.selection.start').call(
      d3.drag().on('drag', event => {
        // this.svg.period.x = event.x;
        // this.translatePeriod();
      })
    )
    ...
    // time ticks
    this.app.ticks = this.svg
      .append('g')
      .classed('ticks', 1)
      .attr('transform', `translate(0,${this.sliderOffsetTop})`)

    this.svg.slider = this.svg
      .append('g')
      .classed('slider', 1)
      .attr('transform', `translate(0, ${this.sliderOffsetTop})`)
    ...
    this.svg.period.interval = this.svg.period
      .append('rect')
    ...
    this.svg.period.blockLeft = this.svg.period
      .append('rect')
      .attr('y', this.sliderOffsetTop + 20)
      .attr('height', this.heightInterval)
    ...
    this.svg.period.blockRight = this.svg.period
      .append('rect')
      .attr('y', this.sliderOffsetTop + 20)
      .attr('height', this.heightInterval)
      .attr('width', this.widthBlock)
