  ...
  <div>
    <b-media v-show="image.thumbnail">
      <div style="width: 128px" slot="aside">
        <b-img fluid-grow slot="aside" v-bind:src="`data:image/png;base64,${image.thumbnail}`" />
      </div>
      ...
      <b-button variant="outline-primary" size="sm" v-on:click="remove">
        {{ $t('actions.remove') }}
      </b-button>
    </b-media>
...
const FILEPOND_SERVICE_PATH = [import.meta.env.VITE_MIDDLELAYER_API_PATH, '/filepond']
  .join('/')
  .replace(/\/\/+/g, '/')
