<template lang="html">
  <div class="topic-item d-flex align-items-center">
    <div class="flex-grow-1">
      ...
      <router-link :to="{ name: 'topic', params: { topic_id: item.id } }" v-html="label" />
      <item-selector :id="item.id" :item="item" type="topic" />
    ...
    <div class="px-2" @click="toggleVisualized">
      <div class="topic-visualized" :class="{ active: visualized }">
...
export default {
  ...
  computed: {
    ...
    visualized() {
      return this.topicsStore.visualizedItemsIndex[this.item.id] != null
    }
  },
  methods: {
    toggleVisualized() {
