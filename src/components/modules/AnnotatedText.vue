...
const flattenChildrenDeep = (children) =>
  children.flatMap((child) =>
    (Array.isArray(child.children) && child.children.length) > 0
      ? [child].concat(flattenChildrenDeep(child.children))
      : child,
  )
...
const getClusterInnerTagStyle = (context, entity, colourMap) => {
  const backgroundColor =
    context.props.selectedClusterId === entity.clusterId
      ? colourMap[entity.clusterId]
      : 'transparent'
...
const renderClusterTags = (h, context, child) => {
  ...
  return getBorderlinePassages(child).map(function (borderlinePassage) {
    var entity = borderlinePassage.entity
    var isLast = borderlinePassage.isLast
    return h(
      'span',
      {
        title: getClusterTitle(context, entity.clusterId),
        class: getClusterClass(isLast),
        style: getClusterTagStyle(
          entity,
          context.props.clusterColours,
          getClusterIndex(entity.clusterId),
        ),
        on: {
          click: function click() {
            onClusterSelected(entity.clusterId)
          },
        },
      },
      [
        h('span', {
          class: 'select',
          style: getClusterInnerTagStyle(context, entity, context.props.clusterColours),
        }),
      ],
    )
  })
...
//     <span
//       class="vertical-line"
//       style={`backgroundColor: ${context.props.clusterColours[child.entity.clusterId]}; right: ${
//         offsetRight * 8
//       }px`}
//     />
//   )
...
export default {
  ...
  render: function (h, context) {
    return (context.props.children || []).map(function (child) {
      if (typeof child === 'string') return child

      if (
        child.entity.kind === 'passage' &&
        child.entity.clusterId !== context.props.selectedClusterId
      ) {
        return [
