      ...
      <b-navbar v-if="$route.params.collection_uid" type="light" variant="light">
        ...
        <section class="ml-auto py-3 text-right">
          <router-link
            ...
            class="m-1"
          >
            <b-button variant="outline-info" size="sm" v-b-modal.confirmDelete
...
export default {
  ...
  methods: {
    ...
    save(collection) {
      if (collection.uid) {
        this.$store
          .dispatch('collections/EDIT_COLLECTION', {
            uid: collection.uid,
    ...
    loadTimeline() {
      ...
      return this.$store
        .dispatch('collections/LOAD_TIMELINE', this.$route.params.collection_uid)
        .then((values) => {
