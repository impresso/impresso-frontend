  ...
  <b-dropdown size="sm" variant="outline-secondary">
    <template v-slot:button-content>
      <div v-if="selectedCollection" class="text-left d-inline-block">
        ...
        <div v-if="selectedCollection.description" class="limit-text">
...
export default {
  ...
  computed: {
    selectedCollection() {
      return this.collections.find(c => c.id === this.active)
    },
    collectionDropdownTitle() {
      const cid = this.collections.find(c => c.id === this.active)
