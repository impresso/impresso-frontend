  ...
  <div class="search-query-explorer" :class="{ 'dark-mode': darkMode }">
    ...
    <div v-else>
      <div class="search-query-explorer-result" v-for="(item, index) in results" :key="index">
        ...
        <article-item
          class="search-query-explorer-article-item p-2 mt-1"
          :item="item"
          show-link
          show-meta
          show-excerpt
...
import { search as searchService, filtersItems as filtersItemsService } from '@/services'
...
export default {
  ...
  computed: {
    /** @returns {Filter[]} */
    enrichedFilters() {
      return this.filtersWithItems.length ? this.filtersWithItems : this.searchQuery.filters
    },
    serializedFilters() {
    ...
    serviceQuery() {
      return {
        filters: this.searchQuery.filters
          .filter(supportedSearchIndexFilters)
          .map(toCanonicalFilter),
        limit: this.limit,
  ...
  methods: {
    getCurrentIndex(index) {
      ...
      return this.$t('numbers.of', {
        index: this.$n(this.limit * (this.paginationCurrentPage - 1) + index + 1),
  ...
  watch: {
    ...
    serializedFilters: {
      async handler(filters, previousSerializedFilters) {
        ...
        // enrich filters
        this.filtersWithItems = await filtersItemsService
          .find({
            query: {
              filters,
            },
