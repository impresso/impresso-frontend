    ...
    <i-layout-section main>
          ...
          <b-navbar-nav v-if="article" class="px-3 py-2 border-right">
            <div v-if="article">
              ...
              <span class="badge">
                ...
                <b-tooltip
                  target="selected-article-language"
                  :title="$t(`buckets.language.${article.language}`)"
                ></b-tooltip>
              </span>
            </div>
          </b-navbar-nav>
          <b-navbar-nav v-if="article && article.type" class="px-3 py-2 border-right">
            <b-form-radio-group v-model="mode" button-variant="outline-primary" size="sm" buttons>
              <b-form-radio value="image"
                >{{ $t('facsimileView') }}&nbsp;<icon name="image"
              /></b-form-radio>
              <b-form-radio value="text" v-bind:disabled="!article"
                ><icon name="align-left" />&nbsp;{{ $t('closeReadingView') }}</b-form-radio
              >
            </b-form-radio-group>
            <small>
      ...
      <div class="d-flex h-100 justify-content-center" v-if="!isContentAvailable && issue">
        <div class="align-self-center">
          ...
          <br />
          <b-button :to="{ name: 'login' }" block size="sm" variant="outline-primary">{{
            $t('actions.login')
          }}</b-button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    loadToC() {
      ...
      return this.$store.dispatch('issue/LOAD_TABLE_OF_CONTENTS').then((articles) => {
